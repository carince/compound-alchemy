...
export default async function handler(
    ...
    if (req.method === "POST") {
        ...
        try {
            ...
            const { answers, score } = JSON.parse(req.body);


            // Check if the user has already completed it.
            if (userData.progress && userData.progress.level1 && userData.progress.level1.quiz && userData.progress.level1.quiz.completed) {
